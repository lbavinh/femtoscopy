...
void PlotCF() {
  ...
  Int_t markerColor[] = {kRed+2,kGreen+2,kBlue+2};
  Int_t markerStyle[] = {kFullCircle, kOpenSquare, kOpenTriangleUp};
  const Int_t nCentBin = 3;
  const Int_t CentralityBinning[nCentBin+1] = {0,10,30,50}; // proxy for centrality binning
  // const Int_t CentralityBinning[nCentBin+1] = {80,70,60,50,40,30,20,10,5,0};
  TH1F *tmp[nCentBin], *tmp2[nCentBin];
  for (Int_t icent(0);icent<nCentBin;icent++) {
    tmp[icent] = (TH1F*) fi->Get(Form("hQinvNum_kt_%i_0",icent));
    tmp2[icent] = (TH1F*) fi->Get(Form("hQinvDen_kt_%i_0",icent));
  ...
  TLatex tex;
  tex.DrawLatexNDC(0.3,0.8,Form("STAR Au+Au #sqrt{s_{NN}} = 27 GeV"));
  tex.SetTextFont(42);
  tex.DrawLatexNDC(0.3,0.7,Form("#pi^{+}#pi^{+}, 0.2 < k_{T} < 0.4 GeV/c"));
  TLegend *leg = new TLegend(0.3,0.3,0.6,0.4);
  leg->SetBorderSize(0);
  leg->SetTextSize(0.04);
  for (Int_t icent(0);icent<nCentBin;icent++) leg->AddEntry(tmp[icent],Form("%i-%i%%",CentralityBinning[icent],CentralityBinning[icent+1]), "p");
  leg->Draw();
  c.SaveAs(Form("CF_PiPi_3cent_kt_02_04.pdf"));
  c.SaveAs(Form("CF_PiPi_3cent_kt_02_04.png"));
